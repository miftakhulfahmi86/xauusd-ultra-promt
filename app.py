...
data['BOS/CHOCH'] = "Ranging"  # Default assignment
valid_data = data.dropna(subset=['EMA20', 'EMA50'])
data.loc[valid_data.index, 'BOS/CHOCH'] = np.where(
    (valid_data['Close'] > valid_data['EMA20']) & (valid_data['EMA20'] > valid_data['EMA50']),
    "BOS",
    np.where(
        (valid_data['Close'] < valid_data['EMA20']) & (valid_data['EMA20'] < valid_data['EMA50']),
        "CHOCH",
